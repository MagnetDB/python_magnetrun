...
def validate_rms_file(filepath):
    """
    ...
    try:
        ...
        required_metadata = ['frequency', 'data_offset', 'sample_width', 'timestamp_bytes']
        ...
        print(f"  ✅ PASSED: All required metadata present")
        print(f"      - Frequency: {reader.metadata['frequency']} Hz")
        print(f"      - Data offset: 0x{reader.metadata['data_offset']:x} ({reader.metadata['data_offset']} bytes)")
        ...
        if expected_width != reader.metadata['sample_width']:
            ...
            print(f"          This is expected for FEPC-AUX-LNCMI (7 unnamed digital variables)")
        ...
        if remainder != 0:
            ...
            print(f"  ✅ PASSED: File size is consistent")
...
def compare_rms_files(filepath1, filepath2):
