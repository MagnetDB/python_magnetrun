    ...
    for system in ['FEPC-LNCMI', 'FEPC-AUX-LNCMI']:
        ...
        if system_dir.exists():
            ...
            if props_file.exists():
                print(f"  ✓ Found EventInfo.properties")
            else:
                print(f"  ⚠ EventInfo.properties not found (optional)")
...
def validate_binary_files(trigger_dir: Path, system: str) -> bool:
    ...
    for tf in files:
        ...
        try:
            ...
            print(f"  ✓ Header read successfully")
    ...
    if config.num_cards == 0:
        ...
        for card in config.cards:
            ...
            if len(card.variable_names) != card.num_channels:
                ...
                print(f"    ✓ Variable count matches channel count")
            
            # Check for calibrations
            if card.calibrations:
                print(f"    ✓ Calibration data available")
...
def validate_data_reading(trigger_dir: Path, system: str) -> bool:
    ...
    try:
        ...
        print(f"  ✓ Full dataset read successfully")
        print(f"    Shape: {data.shape}")
        print(f"    Expected samples: 700,000")
...
def main():
    ...
    parser.add_argument(
        'trigger_dir',
        type=Path,
        help='Path to trigger directory'
    )
