...
if __name__ == "__main__":
    ...
    for i, file in enumerate(input_files):
        ...
        print(
            f"time={timestamp}, timestamp={formatted_timestamp}, ", end="", flush=True
        )
        ...
        for pfile in pupitre_files:
            ...
            pdate, ptime = ptimestamp.split("---")
            # remove seconds
            (ptimestamp, formatted_ptimestamp) = convert_to_timestamp(
                pdate, ptime[:-3], date_format="%Y.%m.%d", time_format="%H:%M"
            )
        ...
        (ochange, oregime, otime, ovalue, ocomponent) = trends(
            mdata,
            "t",
            key=f"{pkey}",
            window=1,
        ...
        (ichange, iregime, itime, ivalue, icomponent) = trends(
            mdata,
            "t",
            key=f"{group}/{channels_dict[args.key]}",
            window=1,
        ...
        for uchannel in uprobes:
            ...
            if f"{ugroup}/{uchannel}" in mdata.getKeys():
                ...
                (uchange, uregime, utime, uvalue, ucomponent) = trends(
                    mdata,
                    "t",
                    key=f"{ugroup}/{uchannel}",
                    window=1,
                    threshold=threshold_dict[uchannel],
                    show=False,
                    save=args.save,
                    debug=args.debug,
                )
        ...
        mdata.plotData(
            x="t", y=f"{group}/{channels_dict[args.key]}", ax=my_ax, normalize=True
        )
        for uchannel in uprobes:
            if f"{ugroup}/{uchannel}" in mdata.getKeys():
                mdata.plotData(
                    x="t", y=f"{ugroup}/{uchannel}", ax=my_ax, normalize=True
                )

        t0 = 0
        for i in range(1, len(oregime)):
            # print(f'axvspan[{i}]: [{t0},{otime[i]}], regime={oregime[i-1]}, color={color_dict[oregime[i-1]]}')
            my_ax.axvspan(t0, otime[i], facecolor=color_dict[oregime[i - 1]], alpha=0.2)
            t0 = otime[i]
        t0 = 0
        for i in range(1, len(iregime)):
            # print(f'axvspan[{i}]: [{t0},{itimes[i]}], regime={iregimes[i-1]}, color={color_dict[iregimes[i-1]]}')
            my_ax.axvspan(t0, itime[i], facecolor=color_dict[iregime[i - 1]], alpha=0.2)
            t0 = itime[i]
        ...
        for i, uregime in enumerate(uregimes):
            ...
            for j in range(1, len(uregime)):
                ...
                my_ax.axvspan(
                    t0, utime[j], facecolor=color_dict[uregime[j - 1]], alpha=0.2
                )
                t0 = utime[j]
        ...
        df["timestamp"] = [
            np.datetime64(t0).astype(datetime.datetime) + datetime.timedelta(0, i * dt)
    ...
    for i, afile in enumerate(archive_files_selected):
        ...
        if args.debug:
            ...
            mdata.plotData(
                x="t", y=f"{group}/{channels_dict[args.key]}", ax=my_ax, alpha=0.2
            )
    ...
    if df_:
        ...
        if args.debug:
            my_ax = plt.gca()
            df_overview.plot(
                x="t", y=channels_dict[args.key], label="Overview", color="r", ax=my_ax
            )
            df_archive.plot(
                x="t",
                y=channels_dict[args.key],
    ...
    for i, pfile in enumerate(natsorted(pupitre_files_selected)):
        ...
        print(
            f't0={mdata.Data["timestamp"].iloc[0]}, len={df.shape[0]}, duration={mdata.getDuration()} s'
        )
        df_.append(df)

        (pchange, pregime, ptime, pvalue, pcomponent) = trends(
            mdata,
            "t",
            key=pupitre_dict[site][args.key],
            window=1,
    ...
    if df_:
        ...
        df_overview.plot(
            x="t", y=args.key, ax=my_ax, marker=".", color="r"
        )  # , linestyle='None'
        legends.append(f"Overview {channels_dict[args.key]}")
        df_overview.plot(
            x="t", y=channels_dict[args.key], ax=my_ax, marker="*", color="b"
        )  # , linestyle='None'
        legends.append(f"pupitre {pupitre_dict[site][args.key]}")
        df_pupitre.plot(
            x="t",
            y=pupitre_dict[site][args.key],
            ax=my_ax,
            marker="o",
            alpha=0.2,
            color="b",
        ...
        df_archive.plot(
            x="t", y=channels_dict[args.key], alpha=0.2, color="b", ax=my_ax
        )
        legends.append(f"Archive {args.key}")
        df_archive.plot(
            x="t", y=args.key, label="Archive", alpha=0.2, color="r", ax=my_ax
        )
        plt.title(channels_dict[args.key])
        plt.title(msg)
        ...
        if args.debug:
            print(
                "pupitre_resampled:",
                ts_pupitre_resampled.size,
                type(ts_pupitre_resampled),
            )
