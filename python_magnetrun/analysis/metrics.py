...
def compute_dtw_distance(
    ...
    try:
        ...
        raise ImportError(
            "dtaidistance is required for DTW. "
...
def dtw_with_paa(
    ...
    try:
        ...
        raise ImportError(
            "Both pyts and dtaidistance are required. "
            f"Missing: {e.name}"
        )
    ...
    distance, paths = dtw.warping_paths(
        series1_paa, series2_paa,
        window=dtw_window, psi=psi
    )
    best_path = dtw.best_path(paths)
...
def plot_tlcc(
    ...
    ax.axvline(
        np.ceil(len(result.correlations) / 2),
        color="k", linestyle="--", label="Center"
    )
    ax.axvline(
        np.argmax(result.correlations),
        color="r", linestyle="--", label="Peak synchrony"
    )
    ax.set(
