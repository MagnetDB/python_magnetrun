...
def synchronize_data(
    ...
    df[time_col] = df.apply(
        lambda row: (row[timestamp_col] - new_t0).total_seconds(),
        axis=1
    )
    
    logger.debug(
        "Synchronized data: shift=%.3f s, source_t0=%s, target_t0=%s",
        timeshift.total_seconds(), source_t0, target_t0
    )
...
def apply_lag_correction(
    ...
    if reference_t0 is not None and time_col in df.columns:
        df.drop([time_col], axis=1, inplace=True)
        df[time_col] = df.apply(
            lambda row: (row[timestamp_col] - reference_t0).total_seconds(),
            axis=1
        )
...
def lag_correlation(
    ...
    correlation = correlate(
        time_series_slice - time_series_slice.mean(),
        trend_slice - trend_slice.mean()
    )
    ...
    logger.debug(
        'lag_correlation: "%s" vs "%s", lag=%d s',
        name_series, name_trend, lag
    )
    
    # Visualization
    if show or save:
        _plot_lag_correlation(
            time_series_slice, trend_slice,
            name_series, name_trend,
            correlation, lags, lag,
            show=show, save=save
        )
...
def _plot_lag_correlation(
    ...
    plt.plot(
        ...
        color="green", marker="o", linestyle="None", alpha=0.2
    )
...
def find_best_matching_regime(
    ...
    for i, regime in enumerate(signature.regimes):
            ...
            for j, ref_regime in enumerate(reference_signature.regimes):
                if ref_regime in ["U", "D"] and j <= len(reference_signature.times) - 2:
                    ...
                    score, tscore, lags = compute_regime_score(
                        regime, values, times,
                        ref_regime, ref_values, ref_times
                    )
        ...
        if best_match is not None:
            best_matches.append(RegimeMatch(
                regime=regime,
...
def check_lag_reliability(
    ...
    if not is_reliable:
        logger.warning(
            ...
            start_ratio, end_ratio, threshold_ratio
        )
