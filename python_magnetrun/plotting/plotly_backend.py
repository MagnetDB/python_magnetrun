...
def _plotly_font_dict(
    ...
    *,
    ...
    label_style : LabelStyle, optional
        Source style.  ``None`` returns ``{"size": fallback_size}``.
    fallback_size : int
        Used when ``label_style`` is ``None`` or ``label_style.size`` is ``None``.
    context : str
        Human-readable name used in the warning message (e.g. ``"xlabel"``).

    Returns
    -------
    dict
        Plotly font dict with supported keys ``size``, ``family``, ``color``.
        ``style`` and ``weight`` from ``LabelStyle`` are not supported by Plotly
