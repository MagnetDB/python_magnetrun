...
def example_trigger_info():
    ...
    print(f"  Total: {trigger_info.total_samples} samples ({trigger_info.total_samples/10000:.1f}s)")
    ...
    for system in ['FEPC-LNCMI', 'FEPC-AUX-LNCMI']:
        ...
        if files:
            ...
            for tf in files:
                ...
                print(f"  {status} Slot {tf.slot} ({tf.card_type}): {tf.filepath.name} ({size_mb:.1f} MB)")
...
def example_read_slot():
    ...
    try:
        ...
        for i in range(min(5, data.shape[1])):
            ...
            channel_data = data[:, i]
            print(f"  {i}: {var_name:12s} - mean: {channel_data.mean():10.3f}, "
