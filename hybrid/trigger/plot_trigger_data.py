...
def plot_multiple_triggers(
    ...
    # Plot each trigger
    for i, (trigger_dir, ax) in enumerate(zip(triggers, axes)):
        ...
        try:
            ...
            # Apply calibration
            if apply_calib and card and card.calibrations and channel < len(card.calibrations):
                calib = card.calibrations[channel]
            ...
            ax.set_title(trigger_info.timestamp.strftime('%H:%M:%S'), 
...
Examples:
  ...
  python plot_trigger_data.py --trigger-dir /data/hybrid/trigger/TRIGGER_2025-11-05_08-16 \\
    ...
    parser.add_argument(
        '--date',
        type=str,
        help='Date in YYYY-MM-DD format (for --all mode)'
    )
    ...
    parser.add_argument(
        '--all',
        action='store_true',
        help='Plot all triggers for specified date'
    )
    ...
    parser.add_argument(
        '--cnv-dir',
        type=Path,
        help='Directory containing CNV calibration files'
    )
