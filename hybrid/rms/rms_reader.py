...
class RMSVariable:
    ...
    def __init__(self, name: str, var_type: str, unit: Optional[str] = None,
...
class RMSFileReader:
    ...
    def read_binary_data(self) -> pd.DataFrame:
        ...
        assert len(data_arrays) == len(self.variables), f"Data arrays length mismatch: {len(data_arrays)} vs {len(self.variables)}"
        print(f'Reading {len(self.variables)} variables', flush=True)
        print(f'Analog: {len([var for var in self.variables if var.is_analog])}', flush=True)
        print(f'Digital: {len([var for var in self.variables if not var.is_analog])}', flush=True)
        ...
        for i in range(num_samples):
            ...
            print(f'Read timestamp[{i}]: {timestamp}', flush=True)
    ...
    def read(self) -> pd.DataFrame:
        ...
        print(f'before parsing header: variables={len(self.variables)}', flush=True)
        if not self.variables:
            self.parse_header()
        print(f'Header parsed: variables={len(self.variables)}', flush=True)
    ...
    def get_variable_info(self) -> pd.DataFrame:
        ...
        if not self.variables:
            self.parse_header()
        print(f'Collecting variable info for {len(self.variables)} variables', flush=True)
        var_info = []
        for var in self.variables:
            var_info.append({
                'name': var.name,
        ...
        print(f'Variable info collected: {len(var_info)} variables ({len(self.variables)})', flush=True)
