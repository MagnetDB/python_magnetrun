...
from rms_reader import RMSFileReader, read_rms_file, get_rms_info
...
def example_plotting(filepath, variables_to_plot):
    ...
    print(f"Plot saved to rms_plot.png")
...
def example_temperature_analysis(filepath):
    ...
    temp_vars = [col for col in df.columns if col.startswith('TT') and not col.endswith('_D1') and not col.endswith('_D2')]
